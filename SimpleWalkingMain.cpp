...
int main(int argc, char* argv[])
{
    ...
    for (double time = 0; time < RunDuration; time += StepSize) {
		...
		write << Insect.Leg.JointX << " " << Insect.Leg.JointY << " ";
        cout << Insect.Leg.FootX << " " << Insect.Leg.FootY << " ";
		write << Insect.Leg.FootX << " " << Insect.Leg.FootY << " ";
        cout << Insect.Leg.FootState << endl;
		write << Insect.Leg.FootState << endl;

    }
    
    // Display the fitness
    cout << "Average velocity = " << Insect.cx/RunDuration << endl;
	write << "Average velocity = " << Insect.cx / RunDuration << endl;
	write.close();
